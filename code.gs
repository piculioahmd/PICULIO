function checkInvoiceStatus() {
  ...
  const sheetNames = ["AWAY", "BEIS", "BRIC`S", "DURAVO", "TUMI", "VICTORINOX"]; // list sheet
  sheetNames.forEach(sheetName => {
    ...
    rows.forEach(row => {
      ...
      const date = row[dateIndex] instanceof Date ? row[dateIndex] : new Date("2999-12-31");
    ...
    Object.values(grouped).forEach(group => {
      ...
      sortedRows.forEach(({ qty, row }) => {
        ...
        } else if (remainingIN >= qty) {
          row[statusIndex] = "✅ OK";
          remainingIN -= qty;
        } else {
          row[statusIndex] = `⚠️ PARTIAL (${remainingIN}/${qty})`;
          remainingIN = 0;
        }
      });
    });

    // Tulis balik ke sheet
